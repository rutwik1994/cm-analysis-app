...
function SupplierCard({
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #E4E4E4", borderRadius: 12, padding: "20px 22px", boxShadow: "0 1px 4px rgba(36,36,36,.06)" }}>
    ...
    <div style={{ background: "#fff", border: "1px solid #E4E4E4", borderRadius: 12, padding: "22px 24px", boxShadow: "0 1px 4px rgba(36,36,36,.06)", marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#F0FDF4", display: "flex", alignItems: "center", justifyContent: "center", font: "700 16px/1 var(--font-display)", color: "#067A46" }}>
...
export default function Page() {
  ...
  const supplierMeta: SupplierMeta[] = [
    { supplier: "BV ProteinsPlus", category: "Protein", cm: "Nicolas Brosens + Laure Montazel" },
    { supplier: "Agri-Fresh BV",   category: "Protein", cm: "Nicolas Brosens + Laure Montazel" },
  ];
    ...
    <div style={{ background: "#F8F8F8", minHeight: "100vh", padding: "32px 24px", fontFamily: "var(--font-body)" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, font: "400 13px/18px var(--font-body)", color: "#676767" }}>
          ...
          <span>/</span>
          <span style={{ color: "#242424", fontWeight: 600 }}>BENELUX</span>
        </div>

        {/* Page header */}
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ font: "700 28px/36px var(--font-display)", color: "#242424", margin: 0 }}>BENELUX Category Management</h1>
            <p style={{ font: "400 14px/20px var(--font-body)", color: "#676767", margin: "6px 0 0" }}>
              Protein only — contract spend overview &amp; negotiation prep
            </p>
          </div>
        ...
        <div style={{ background: "#FAFAFA", border: "1px solid #E4E4E4", borderRadius: 10, padding: "12px 16px", marginBottom: 24, font: "400 13px/18px var(--font-body)", color: "#676767", display: "flex", alignItems: "center", gap: 8 }}>
          <span>ℹ️</span>
          No Bakery or Grocery contracts active in the BENELUX market for this contract period. Only Protein contracts are in scope.
        </div>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 28 }}>
          ...
          <KpiCard label="Budget Utilisation" value={`${metrics.budgetUtilizationPct}%`} sub={metrics.budgetUtilizationPct >= 90 ? "⚠ High risk" : metrics.budgetUtilizationPct >= 75 ? "Monitor closely" : "On track — under-delivery watch"} tone={metrics.budgetUtilizationPct >= 90 ? "danger" : metrics.budgetUtilizationPct >= 75 ? "warning" : "positive"} />
          <KpiCard label="Suppliers" value={String(metrics.supplierCount)} sub="Protein contracts only" tone="neutral" />
          <KpiCard label="At-Risk Suppliers" value={String(metrics.atRiskSuppliers)} sub={metrics.atRiskSuppliers > 0 ? "≥80% budget used" : "All within budget"} tone={metrics.atRiskSuppliers > 0 ? "warning" : "positive"} />
        </div>

        {/* Under-delivery note */}
        <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "12px 16px", marginBottom: 24, font: "400 13px/18px var(--font-body)", color: "#1E40AF", display: "flex", alignItems: "flex-start", gap: 8 }}>
          <span style={{ fontSize: 16 }}>📉</span>
          <div>
            <strong>Under-delivery pattern detected.</strong> BENELUX Protein suppliers (Chicken Thigh 300g) are tracking below budget — both BV ProteinsPlus and Agri-Fresh BV are at ~55% utilisation. Review volume commitments and consider reforecast before contract renewal.
          </div>
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <CmSection name="Nicolas Brosens" suppliers={["BV ProteinsPlus", "Agri-Fresh BV"]} rows={marketRows} marketLabel={MARKET} />
          <CmSection name="Laure Montazel"  suppliers={["BV ProteinsPlus", "Agri-Fresh BV"]} rows={marketRows} marketLabel={MARKET} />
        </div>

        {/* Supplier Performance + Negotiation Prep */}
        <div>
          <h2 style={{ font: "700 18px/24px var(--font-display)", color: "#242424", margin: "0 0 16px" }}>Supplier Performance &amp; Negotiation Prep</h2>
