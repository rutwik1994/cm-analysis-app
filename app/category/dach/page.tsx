...
function UtilBar({ actual, awarded, color }: { actual: number; awarded: number; color: string }) {
  ...
  return (
    <div>
      ...
      <div style={{ font: "400 10px/14px var(--font-body)", color: "#9CA3AF", marginTop: 3 }}>
        of {fmtFull(awarded)} awarded
      </div>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <span style={{
            font: "600 11px/14px var(--font-body)", padding: "3px 8px", borderRadius: 99,
            background: badge.bg, color: badge.color,
          }}>{badge.label}</span>
          <span style={{
            font: "500 10px/14px var(--font-body)", padding: "2px 7px", borderRadius: 99,
      ...
      {/* Last update */}
      <div style={{ font: "400 10px/14px var(--font-body)", color: "#9CA3AF", marginTop: 10 }}>
      ...
      <div style={{ marginTop: 16 }}>
        <button
          ...
          style={{
            font: "600 12px/18px var(--font-body)", padding: "8px 14px", borderRadius: 8,
            border: "1px solid #067A46", background: loadingBrief ? "#F0FDF4" : "#fff",
            color: "#067A46", cursor: loadingBrief ? "not-allowed" : "pointer",
        ...
        >
          {loadingBrief ? (
            <>
              <span style={{ display: "inline-block", width: 12, height: 12, border: "2px solid #067A46", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
              Generating…
            </>
          ) : (
      ...
      {brief && (
        <div style={{
          marginTop: 16, padding: "16px 18px", background: "#F8FFF8", border: "1px solid #BBF7D0",
          borderRadius: 10, font: "400 13px/20px var(--font-body)", color: "#242424", whiteSpace: "pre-wrap",
        }}>
          {brief}
        </div>
      )}
...
function CmSection({ name, suppliers, rows }: {
  name: string;
  suppliers: string[];
  rows: SpendRow[];
}) {
  ...
  return (
    <div style={{
      background: "#fff", border: "1px solid #E4E4E4", borderRadius: 12,
      padding: "22px 24px", boxShadow: "0 1px 4px rgba(36,36,36,.06)", marginBottom: 16,
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            font: "700 16px/1 var(--font-display)", color: "#067A46",
          }}>
          ...
          <div>
            ...
            <div style={{ font: "400 12px/16px var(--font-body)", color: "#676767", marginTop: 2 }}>
              Category Manager · {suppliers.length} supplier{suppliers.length !== 1 ? "s" : ""} in DACH
            </div>
          </div>
        </div>
        <span style={{
          font: "600 12px/16px var(--font-body)", padding: "5px 12px", borderRadius: 99,
...
export default function Page() {
  ...
  const supplierMeta: SupplierMeta[] = [
    { supplier: "AmoreFood GmbH BAK",  category: "Bakery",  cm: "Manon Turpaud" },
    { supplier: "Lika Bakery BV",      category: "Bakery",  cm: "Manon Turpaud" },
    { supplier: "Barilla GmbH",        category: "Grocery", cm: "Iryna Zender" },
    { supplier: "TortiPasta GmbH",     category: "Grocery", cm: "Iryna Zender" },
    { supplier: "Müller Fleisch GmbH", category: "Protein", cm: "Nicolas Brosens + Laure Montazel" },
    { supplier: "Wiesenhof Südwest",   category: "Protein", cm: "Nicolas Brosens + Laure Montazel" },
  ];
    ...
    <div style={{ background: "#F8F8F8", minHeight: "100vh", padding: "32px 24px", fontFamily: "var(--font-body)" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, font: "400 13px/18px var(--font-body)", color: "#676767" }}>
          ...
          <span>/</span>
          <span style={{ color: "#242424", fontWeight: 600 }}>DACH</span>
        </div>

        {/* Page header */}
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ font: "700 28px/36px var(--font-display)", color: "#242424", margin: 0 }}>DACH Category Management</h1>
            <p style={{ font: "400 14px/20px var(--font-body)", color: "#676767", margin: "6px 0 0" }}>
              Bakery · Grocery · Protein — contract spend overview &amp; negotiation prep
            </p>
          </div>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 28 }}>
          ...
          <KpiCard
            ...
            sub={metrics.budgetUtilizationPct >= 90 ? "⚠ High risk" : metrics.budgetUtilizationPct >= 75 ? "Monitor closely" : "On track"}
            tone={metrics.budgetUtilizationPct >= 90 ? "danger" : metrics.budgetUtilizationPct >= 75 ? "warning" : "positive"}
          />
          ...
          <KpiCard
            ...
            sub={metrics.atRiskSuppliers > 0 ? "≥80% budget used" : "All within budget"}
            tone={metrics.atRiskSuppliers > 0 ? "warning" : "positive"}
          />
        </div>

        {/* Category Manager Sections */}
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ font: "700 18px/24px var(--font-display)", color: "#242424", margin: "0 0 16px" }}>Category Managers</h2>
          <CmSection name="Manon Turpaud"   suppliers={["AmoreFood GmbH BAK", "Lika Bakery BV"]}              rows={marketRows} />
          <CmSection name="Iryna Zender"    suppliers={["Barilla GmbH", "TortiPasta GmbH"]}                   rows={marketRows} />
          <CmSection name="Nicolas Brosens" suppliers={["Müller Fleisch GmbH", "Wiesenhof Südwest"]}          rows={marketRows} />
          <CmSection name="Laure Montazel"  suppliers={["Müller Fleisch GmbH", "Wiesenhof Südwest"]}          rows={marketRows} />
        </div>

        {/* Supplier Performance + Negotiation Prep */}
        <div>
          <h2 style={{ font: "700 18px/24px var(--font-display)", color: "#242424", margin: "0 0 16px" }}>
            Supplier Performance &amp; Negotiation Prep
          </h2>
          <p style={{ font: "400 13px/18px var(--font-body)", color: "#676767", margin: "0 0 20px" }}>
            ...
            {supplierMeta.map(({ supplier, category, cm }) => {
              ...
              return (
                <SupplierCard
                  ...
                  supplier={supplier}
                  category={category}
                  actual={entry.actualEur}
                  awarded={entry.awardedEur}
                  cm={cm}
                  lastWeek={lastWeekMap[supplier] ?? "—"}
                  onGenerateBrief={() => generateBrief(supplier, category, cm)}
                  brief={briefs[supplier]}
                  loadingBrief={loadingBrief[supplier] ?? false}
                />
              );
            })}
          </div>
